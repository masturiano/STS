...
class PDF extends FPDF
{
	...
	function Content($arrSOA) {
		...
		$this->SetFont('Helvetica', '', '8');
		$this->Cell(39,4,$arrSOA['stsRefno'].' - '.$arrSOA['nbrApplication'],1,0,'C',0);
		...
		$this->SetFont('Helvetica', '', '8');
		$this->Cell(30,4,"INVOICE NO.",0,0,'L');
		$this->Cell(68,4,"STORE",0,0,'L');
	...
	function Content2($arrSOA2) {
		...
		foreach($arrSOA2 as $val){
			$this->SetFont('Helvetica', '', '8');
			$this->Cell(30,4,$val['stsNo'],0,0,'L');
			$this->Cell(68,4,$val['strNam'],0,0,'L');
		...
		$this->Cell(98,4,"TOTAL AMOUNT",0,0,'L');
	...
	function Content3($arrSOA) {
		...
		$this->SetFont('Helvetica', 'UI', '7');
		$this->Cell(38,4,"we will apply this account for invoice Deduction after 30 days without further notice.",0,1,'L');
		$this->SetFont('Helvetica', 'I', '7');
...
$arrFundsInfo = $transObj->getSOAInfo($_GET['refNo']);
$arrFundsInfoArr = $transObj->getSOAInfo2($_GET['refNo']);
