...
################### INCLUDE FILE #################
	...
	$worksheet->write(0,0,$type[$_GET['cmbType']]." Summary per Store per Product Category From ".date('m/d/Y',strtotime($_GET['txtDateFrom']))." to ".date('m/d/Y',strtotime($_GET['txtDateTo'])),$headerFormat);
	for($i=1;$i<=1;$i++) 
	{
	...
	$worksheet->setColumn(0,40,15);
		...
		$arrCountMonth = $stsSummaryPerStorePerProdCatObj ->getCountMonth($_GET['txtDateFrom'],$_GET['txtDateTo']);
		...
		$existGroup = $stsSummaryPerStorePerProdCatObj->getExistGroup();
		$ctrH = 1;
		...
		$arrDet = $stsSummaryPerStorePerProdCatObj ->stsSummaryPerStorePerProdCat($_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbType']);
		...
		foreach ($arrDet as $valDet) {
			...
			$worksheet->write($ctr,1,$valDet['brnDesc'],$detail3);
