...
switch($_GET['action']){	
	...
	case 'PrintXls':
			echo "window.open('monthlyStsSummaryPerSupplier_XLS.php?{$_SERVER['QUERY_STRING']}');";
	exit();
	break;
...
<script type="text/javascript">
	$(function(){		
		...
		$("#txtSup").autocomplete({
			source: "monthlyStsSummaryPerSupplier.php?action=searchSupplier",
			minLength: 1,
		...
		$("#printInq").click( function (){
			...
			if (valDateStartEnd(dateStart,dateEnd,'txtDateFrom','txtDateTo')){
				$.ajax({
					url: 'monthlyStsSummaryPerSupplier.php',
					type: "GET",
					data: $("#formInq").serialize()+'&action=action=PrintXls',
...
<h2 class="ui-widget-header ui-corner-all" style="padding:5px;">MONTHLY STS SUMMARY PER SUPPLIER</h2>
