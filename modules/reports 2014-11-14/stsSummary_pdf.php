...
class PDF extends FPDF
{
		...
		$reportsObj = new reportsObj();
			$this->Ln(3);
				...
				foreach ($arrH as $valH) {
					...
					$this->Cell(15,5,number_format($valH['stsVatAmt'],2),0,0,'R');
					}else{
					$this->Cell(15,5,'0',0,0,'R');
					}
					$this->Cell(4,5,'',0,0,'L');
					$this->Cell(20,5,date('m/d/Y',strtotime($valH['dateApproved'])),0,0,'L');
					$this->Cell(30,5,$valH['payMode'],0,0,'L');
					$this->Cell(31,5,substr($valH['dept'],0,4)."-".substr($valH['cls'],0,4)."-".substr($valH['subCls'],0,4),0,0,'L');
					$this->Cell(58,5,substr($valH['stsRemarks'],0,40),0,1,'L');
	...
	function Header() {
		...
		$this->Cell(22,8,'Vat Amount','BT',0,'L');
