...
################### INCLUDE FILE #################
	...
	$worksheet->write(0,0,$type[$_GET['cmbType']]."Promo Fund Summary per Supp per Product Category From ".date('m/d/Y',strtotime($_GET['txtDateFrom']))." to ".date('m/d/Y',strtotime($_GET['txtDateTo'])),$headerFormat);
	for($i=1;$i<=3;$i++) 
	{
	...
	$worksheet->setColumn(0,10,20);
		...
		$arrDet = $stsSummaryPerSuppPerProdCatObj ->pfSummaryPerSuppPerProdCat($_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbType']);
		...
		foreach ($arrDet as $valDet) {
			...
			$worksheet->write($ctr,0,$valDet['suppCode'],$detail3);
			$suppName = $stsSummaryPerSuppPerProdCatObj ->getSuppName($valDet['suppCode']);
			$worksheet->write($ctr,1,$suppName['suppName'],$detail3);
