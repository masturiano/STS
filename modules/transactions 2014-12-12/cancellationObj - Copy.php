...
FROM         tblStsHdr INNER JOIN
	...
	function getAprrovedSTSDtl($refNo){
		$sql = "SELECT   tblStsDtl.stsRefno,  tblStsDtl.stsNo, tblStsDtl.stsAmt, sql_mmpgtlib.dbo.APSUPP.ASNUM, sql_mmpgtlib.dbo.APSUPP.ASNAME, tblStsDtl.strCode, tblBranches.brnDesc, 
...
FROM         tblStsDtl INNER JOIN
                      ...
                      tblBranches ON tblStsDtl.strCode = tblBranches.strCode
	...
	function cancelSTSWHOLE($refNo,$reason,$cancelDate){
		...
		$sqlInsertCancelled = "INSERT INTO tblcancelledsts (stsNo, stsSeq, stsRefNo, compCode, strCode, suppCode, stsType, stsPaymentMode, stsDept, stsCls, stsSubCls, grpCode, stsApplyDate, stsStrAmt,cancelDate, cancelCode, cancelledBy, effectivityDate) 
		SELECT     tblStsDtl.stsNo, '0' AS seq, tblStsDtl.stsRefno, tblStsDtl.compCode, tblStsDtl.strCode, tblStsHdr.suppCode, tblStsHdr.stsType, tblStsHdr.stsPaymentMode, 
                      tblStsHdr.stsDept, tblStsHdr.stsCls, tblStsHdr.stsSubCls, tblStsHdr.grpCode, tblStsHdr.applyDate,tblStsDtl.stsAmt,  '".date('m/d/Y',strtotime($cancelDate))."', '".$lastId."', '".$_SESSION['sts-userId']."', '".date('m/d/Y',strtotime($cancelDate))."'
FROM         tblStsDtl INNER JOIN
	...
	function cancelSTSREOPEN($refNo,$reason,$cancelDate){
		...
		$sqlInsertCancelled = "INSERT INTO tblcancelledsts (stsNo, stsSeq, stsRefNo, compCode, strCode, suppCode, stsType, stsPaymentMode, stsDept, stsCls, stsSubCls, grpCode, stsApplyDate, stsStrAmt,cancelDate, cancelCode, cancelledBy, effectivityDate) 
		SELECT     tblStsDtl.stsNo, '0' AS seq, tblStsDtl.stsRefno, tblStsDtl.compCode, tblStsDtl.strCode, tblStsHdr.suppCode, tblStsHdr.stsType, tblStsHdr.stsPaymentMode, 
                      tblStsHdr.stsDept, tblStsHdr.stsCls, tblStsHdr.stsSubCls, tblStsHdr.grpCode, tblStsHdr.applyDate,tblStsDtl.stsAmt,  '".date('m/d/Y')."', '".$lastId."', '".$_SESSION['sts-userId']."',  '".date('m/d/Y',strtotime($cancelDate))."'
FROM         tblStsDtl INNER JOIN
                      tblStsHdr ON tblStsDtl.stsRefno = tblStsHdr.stsRefno WHERE  tblStsDtl.stsRefno = '$refNo';";
		if($trans){
			$trans = $this->execQry($sqlInsertCancelled);	
	...
	function cancelStsDtl($arr){
		...
		for($i=0;$i<=(int)$arr['hdCtr2'];$i++) {
			if((int)$arr["switcher_".$i]==1){
				...
				$sqlInsertCancelled = "INSERT INTO tblcancelledsts (stsNo, stsSeq, stsRefNo, compCode, strCode, suppCode, stsType, stsPaymentMode, stsDept, stsCls, stsSubCls, grpCode, stsApplyDate,cancelledBy,cancelDate,cancelCode,effectivityDate,stsStrAmt) SELECT stsNo, stsSeq, stsRefNo, compCode, strCode, suppCode, stsType, stsPaymentMode, stsDept, stsCls, stsSubCls, grpCode, stsApplyDate,  '".$_SESSION['sts-userId']."', '".date('m/d/Y')."', '".$lastId."','".$cancelDate."', stsApplyAmt FROM tblstsapply WHERE  stsRefno = '".$arr['hdnRefno']."' AND stsApplyDate >= '$cancelDate' AND stsNo = '".$arr['stsNo_'.$i]."';";
				if($trans){
					$trans = $this->execQry($sqlInsertCancelled);	
				}
