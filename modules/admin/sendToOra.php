...
switch($_GET['action']) {
	case "process":
		/*if($EODobj->ExtractTblSTShdr()){
			...
			if($_GET['cmbTran'] == 0){
				if($EODobj->apUploadToOracle($_GET['txtDateFrom'],$_GET['txtDateTo'])){
			...
			if($_GET['cmbTran'] == 2){
				if($EODobj->arUploadToOracle($_GET['txtDateFrom'],$_GET['txtDateTo'])){
					echo "dialogAlert('STS successfully Uploaded.');\n";
				}else{
					echo "dialogAlert('Upload Success.');\n";
				}
	...
	<form id="formUser" name="formUser">
        <table width="100%" border="0" cellspacing="0" cellpadding="0">  <tr>
        	<tr>
				...
				<td>Type: <? $EODobj->DropDownMenu($arrTran,'cmbTran','','class="selectBox"'); ?></td>
