...
class EODobj extends commonObj {
	...
	function apUploadToOracle($dtFrom,$dtTo){
			
			$sqlTruncDtsloop_x = "
			truncate table ORA..dtsloop_x
			";
			$this->execQry($sqlTruncDtsloop_x);
...
FROM         tblStsDlyApHist left JOIN
		...
		function arUploadToOracle($dtFrom,$dtTo){
			
			$sqlTruncArInvoice = "
			truncate table ORA..ar_invoice
			";
			$this->execQry($sqlTruncArInvoice);
...
FROM         tblStsDlyArHist LEFT OUTER JOIN
					...
					if($valAr['compCode']==700){
                        ...
                        $fileFolder = "PPCI";
                        $fileCode = "PG";
                        if(($valAr['compCode']>800 && $valAr['compCode']<809) || ($valAp['compCode']==809)){
                            $newCompCode = 101;
                        }else{
