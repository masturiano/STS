...
class PDF extends FPDF
{
	...
	function Content() {
		...
		$arrComp = $transObj->getMaxCom2($_GET['refNo']);
		...
		foreach($arrStsPar as $row){
			...
			$this->Cell($w[3],6,number_format($row['stsVatAmt'],2),'LR',0,'R');
			$this->Cell($w[4],6,number_format($row['stsAmt']+$row['stsVatAmt'],2),'LR',0,'R');
			$this->Ln();
		}
		//Closure line
	...
	function Header() {
        ...
        */
		...
		$this->Cell(20,5,number_format($arrSTS['stsAmt2'],2),0,1,'L');
		...
		$this->Cell(20,5,'Subject to issuance of Official Receipt (OR)',0,1,'L');
		}else{
		$this->Cell(25,5,'*',0,0,'L');
		$this->Cell(20,5,'Subject to issuance of Collection Receipt (CR)',0,1,'L');	
		}	
		
		$this->Ln(3);
		$w=array(30,50,35,35,35);
		//Header
		$header=array('STS No','Store','Amount','VAT','Gross Amt');
		$this->Cell(10,6,"",0,0,'L');	
