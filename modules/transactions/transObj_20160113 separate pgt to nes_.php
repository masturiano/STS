...
class transObj extends commonObj {
	...
	function getPaginatedDispSTS($sidx,$sord,$start,$limit){	
        ...
        if($_SESSION['sts-company'] == 1){
            $libraryMaster = "sql_mmpgtlib";
        }
        else{
            $libraryMaster = "sql_mmneslib";
        }
        ...
        $sql = "SELECT TOP  $limit
            ...
            END AS stsStat,
            $libraryMaster.dbo.APSUPP.ASNAME as suppName,
            ...
            FROM
              tblStsHdr LEFT JOIN
                      $libraryMaster.dbo.APSUPP ON tblStsHdr.suppCode = $libraryMaster.dbo.APSUPP.ASNUM
	...
	function findSupplier($terms){
		...
		$sql = "SELECT     TOP 10 $libraryMaster.dbo.APSUPP.ASNUM AS suppCode, $libraryMaster.dbo.APSUPP.ASNAME AS suppName, 
                      $libraryMaster.dbo.APADDR.AACONT AS contactPerson
FROM         $libraryMaster.dbo.APSUPP LEFT OUTER JOIN
                      $libraryMaster.dbo.APADDR ON $libraryMaster.dbo.APSUPP.ASNUM = $libraryMaster.dbo.APADDR.AANUM
WHERE     ($libraryMaster.dbo.APSUPP.ASNUM LIKE '%$terms%') AND ($libraryMaster.dbo.APSUPP.ASNAME NOT LIKE '%NTBU%')
AND ($libraryMaster.dbo.APSUPP.ASTYPE = 1)
OR
($libraryMaster.dbo.APSUPP.ASNAME LIKE '%$terms%') AND ($libraryMaster.dbo.APSUPP.ASNAME NOT LIKE '%NTBU%')
AND ($libraryMaster.dbo.APSUPP.ASTYPE = 1)
";
		return $this->getArrRes($this->execQry($sql));
	}
	...
	function saveHeader($arr){
		...
		if($arr['cmbClass']==''){$cls=0;}else{$cls=$arr['cmbClass'];}
		if($arr['cmbSubClass']==''){$scls=0;}else{$scls=$arr['cmbSubClass'];}
		if($arr['txtSTSAmount']==''){$amt=0;}else{$amt=$arr['txtSTSAmount'];}
		...
		$sqlInsert = "INSERT INTO tblStsHdr (stsRefNo, suppCode, stsDept, stsCls, stsSubCls, stsAmt, stsRemarks, 
			...
			'{$arr['txtNoApplications']}', '{$arr['txtApDate']}','".$_SESSION['sts-userId']."', '".$now."', '".$_SESSION['sts-grpCode']."', 'O', '1', 'Y', '{$arr['txtRep']}', '{$arr['txtRepPos']}','".$_SESSION['sts-strCode']."','{$arr['vatTag']}' $values)";
	...
	function getRegSTSInfoAssoc($refNo){
		...
		$sql = "
            SELECT     
                ...
                $libraryMaster.dbo.APSUPP.ASNAME as suppName, 
                tblStsHdr.vatTag, 
                tblStsHdr.holdingDate, 
                tblStsHdr.onHoldTag, 
                tblStsHdr.onHoldCount
		    FROM         
	...
	function getContractInfo($refNo){
		...
		$sql = "
            SELECT     
                ...
                tblStsHdr.stsRemarks, 
                DATEADD(month, 
                tblStsHdr.nbrApplication - 1, 
                tblStsHdr.applyDate) AS endDate, 
                DATEADD(month, 
                tblStsHdr.nbrApplication, 
                tblStsHdr.applyDate) AS endDate2, 
                tblUsers.fullName, 
                tblStsHdr.contactPerson, 
                tblStsHdr.contactPersonPos, 
                $libraryMaster.dbo.APSUPP.ASNAME AS suppName, 
                $libraryMaster.dbo.APADDR.AAADD1 AS add1, 
                $libraryMaster.dbo.APADDR.AAADD2 AS add2, 
                $libraryMaster.dbo.APADDR.AAADD3 AS add3, 
...
FROM         tblStsHdr INNER JOIN
                      ...
                      $libraryMaster.dbo.APADDR ON tblStsHdr.suppCode = $libraryMaster.dbo.APADDR.AANUM 
					  INNER JOIN
                      $libraryMaster.dbo.APSUPP ON tblStsHdr.suppCode = $libraryMaster.dbo.APSUPP.ASNUM 
...
FROM         tblStsEnhanceDtl INNER JOIN
	...
	function getSTSPrint($refNo){
        ...
        $sql = "SELECT     tblStsHdr.stsRefno, tblUsers.fullName  as approvedBy, tblStsHdr.dateApproved, $libraryMaster.dbo.APSUPP.ASNAME AS suppName, tblStsHdr.stsType, 
...
FROM         tblStsHdr LEFT OUTER JOIN
                      ...
                      $libraryMaster.dbo.APSUPP ON tblStsHdr.suppCode = $libraryMaster.dbo.APSUPP.ASNUM LEFT JOIN
                      tblUsers tblUsers_1 ON tblStsHdr.enteredBy = tblUsers_1.userId LEFT JOIN
                      tblGroup ON tblUsers.grpCode = tblGroup.grpCode
                      WHERE tblStsHdr.stsRefno = '$refNo'";    
        return $this->getSqlAssoc($this->execQry($sql));
	}
...
WHERE     (tblStsApply.suppCode IN
                          (SELECT     asnum
                            FROM          $libraryMaster..apsupp
                            WHERE      asname LIKE '%NTBU%') OR
                      tblStsApply.suppCode IN
                          (SELECT     adnum
                            FROM          $libraryMaster..apsupa
...
AND stsComp = '{$val['stsComp']}'";
	...
	function getSOAInfo($refNo,$stsSeq) {
        ...
        left join libraryMaster.dbo.APSUPP s on s.asnum = h.suppCode
	...
	function getSOAInfo2($refNo,$stsSeq) {
        ...
        left join libraryMaster.dbo.APSUPP s on s.asnum = ar.suppCode
        left join libraryMaster.dbo.TBLSTR b on b.strNum = ar.strCode
        where 
        ar.stsPaymentMode = 'C'
	...
	*/
	...
	function checkOnHold(){
		$sql = "SELECT * FROM tblStsHdr WHERE holdingDate <= '".date('m/d/Y')."' AND onHoldTag = 'Y'";	
		return $this->getArrRes($this->execQry($sql));	
	}
}	
?>
