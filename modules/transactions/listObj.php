...
class listObj extends commonObj {
	...
	function saveHeader($arr){
		...
		$sqlInsert = "INSERT INTO tblStsHdr (stsRefNo, suppCode, stsDept, stsCls, stsSubCls, stsRemarks, 
			...
			VALUES 
			('$tempRefNo', '{$arr['hdnSuppCode']}', '1', '0', '0', '{$arr['txtRemarks']}', '{$arr['cmbPayType']}', NULL, '$nbr', '{$arr['txtApDate']}','".$_SESSION['sts-userId']."', '".$now."', '".$_SESSION['sts-grpCode']."', 'O', '2', 'Y', '{$arr['txtRep']}', '$pos','".$_SESSION['sts-strCode']."','{$arr['vatTag']}' $values)";
...
WHERE     (tblStsDtl.stsRefno = $refNo)";
	...
	function updateHeader($arr){
		...
		if((int)$this->countDetail($arr['refNo']) > 1){
			if($arr['vatTag']=='Y'){
				...
				echo $sqlUpdateDtl = "
                    UPDATE tblStsDtl 
                    SET 
                        stsAmt = '{$arr['txtAmtPerStr']}', 
                        stsVatAmt = '".$arr['txtAmtPerStr']*$vat."', 
				        stsEwtAmt = '".($arr['txtAmtPerStr']*$ewt) * -1 ."' 
                    WHERE stsRefno = '{$arr['refNo']}'
                ";	
			}
            else
        ...
        if((int)$this->countDetail($arr['refNo']) > 1){
		    if($trans){
			    echo $trans = $this->execQry($sqlUpdateDtl);	
		    }
        }
