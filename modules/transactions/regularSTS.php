...
switch($_GET['action']){	
	...
	case 'getInfo':
        ...
        if(date('m/d/Y',strtotime($val['holdingDate'])) == '01/01/1970'){
            echo "$('#ckHold').removeAttr('disabled', false);\n"; 
        }else{
            if((int)$val['onHoldCount'] > 2){     
                echo "$('#txtHoldDate').val('".date('m/d/Y',strtotime($val['holdingDate']))."');\n"; 
                echo "$('#txtHoldDate').attr('readonly', true);\n";
                echo "$('#ckHold').attr('disabled', true);\n"; 
            }else{
                echo "$('#txtHoldDate').val('".date('m/d/Y',strtotime($val['holdingDate']))."');\n"; 
                echo "$('#txtHoldDate').removeAttr('readonly', false);\n";
                echo "$('#ckHold').removeAttr('disabled', false);\n";
            }
        }
        ...
        /* Commented by Mydel
		if($val['onHoldTag']=='Y'){
			echo "$('#ckHold').attr('checked', true);";	
			echo "$('#txtHoldDate').val('".date('m/d/Y',strtotime($val['holdingDate']))."');\n";
			echo "$('#txtHoldDate').attr('disabled',false);";
		}else{
			echo "$('#ckHold').attr('checked', false);";
			echo "$('#txtHoldDate').attr('disabled',true);";   
		}
        End of comment */    
	exit();
	break;
	...
	case 'UpdateHdr':
        ...
        $val = $transObj->getRegSTSInfoAssoc($_GET['refNo']);
        $onHoldCount = "select onHoldCount From tblStsHdr where stsrefno = {$_GET['refNo']}";
        $getOnHoldCount = $transObj->getSqlAssoc($transObj->execQry($onHoldCount));
        
		if($transObj->updateHeader($_GET)){
            
            if($getOnHoldCount['onHoldCount'] > 1){  
                echo "$('#txtHoldDate').attr('disabled', true);\n";
                echo "$('#ckHold').attr('disabled', true);\n";  
            }else{
                echo "$('#txtHoldDate').removeAttr('readonly', false);\n";
                echo "$('#txtHold').removeAttr('disabled', false);\n";
            } 
    ...
    case 'printSTS':
		if($transObj->checkPayment($_GET['refNo'])=='C'){
			echo "window.open('report/stsContent_pdfCollection.php?{$_SERVER['QUERY_STRING']}');";
		}else{
	...
	case 'printSOA':
		...
		if($transObj->createTempTable((int)$_GET['refNo'])){
			if($transObj->checkPayment($_GET['refNo'])=='C'){
				if($transObj->getSequence((int)$_GET['refNo'])){
					$seq = $transObj->getSequence((int)$_GET['refNo']);
					$addMonth = 1;
					foreach($seq as $val){
						$addMonth++;
						//echo "window.open('report/soa_pdf.php?{$_SERVER['QUERY_STRING']}');";
						echo "window.open('report/soa_pdf.php?refNo={$_GET['refNo']}&seq={$val['stsSeq']}');";
					}
				}else{
					//echo "dialogAlert('Error printing SOA.','dialogAlertFunds','dialogMsgFunds',150);";
					echo "dialogAlert('Error printing SOA!');";
				}
			}else{
				echo "dialogAlert('SOA applicable for collection only!');";
			}
		}
	...
	case 'checkOnHold':
		$arr = $transObj->checkOnHold();
		if(count($arr)>0){
			$a = '';
			foreach($arr as $val){
				$a = $a.$val['stsRefno'].",";
			}
			echo "dialogAlert('On Hold STS DUE: sts ref no ".$a."');";
		}
	exit();
	break;
}
...
$(function(){
	...
	$("#txtHoldDate").attr("disabled",true);
	$('#txtApDate,#txtHoldDate').datepicker();
	$("#stsTable").jqGrid({ 
		...
		onSelectRow: function(id){ 
			...
			if(stat == 'OPEN'){
				...
				$("#soa").removeClass("link");
				$("#soa").addClass("disable");
...
function saveHdr(){ 
	...
	if(validateInputs()){
		$.ajax({
			...
			data: $("#formSTS").serialize()+'&action=saveHdr&vatTag='+vatTag+'&holdTag='+holdTag,
...
function updateHdr(){
	...
	if(validateInputs()){
		$.ajax({
			...
			type: "GET",
			traditional: true,
			data: $("#formSTS").serialize()+'&action=UpdateHdr&refNo='+stsRefNo+'&compCode='+compCode+'&vatTag='+vatTag+'&holdTag='+holdTag,
			success: function(msg){
...
function validateInputs() {
	...
	if ($('#ckHold').is(":checked")){
		...
		if (validateString($("#txtRemarks"))== false)
			val = false;	
		if (validateString($("#txtHoldDate"))== false)
			val = false;
	}else{
...
function SOA(){
	...
	$.ajax({
		url: 'regularSTS.php',
		type: "GET",
		data: 'action=printSOA&refNo='+stsRefNo,
		beforeSend: function() {
			ProcessData('Printing SOA...','Open');
		},
		success: function(Data){
			ProcessData('','Close');
			eval(Data);
...
function enableDisableHold(){
	if ($('#ckHold').is(":checked")){
		$("#txtHoldDate").attr("disabled",false);
		$("#txtHoldDate").val("");
	}else{
		$("#txtHoldDate").attr("disabled",true);
		$("#txtHoldDate").val("");
...
<span class="style23, disable" id="contract" title="Print" onclick="">
	<img src="../../images/print.png" title="Print" /> Print Contract
</span>

&nbsp;&nbsp;&nbsp;&nbsp;
<span class="style23, disable" id="soa" title="Print" onclick="">
	<img src="../../images/print.png" title="Print" /> Print SOA
</span>
<!--
&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<div style=" visibility:hidden;  overflow:hidden;">
        <div id='dialogAdd' title='Add'>
            ...
            <table width="100%" border="0" cellspacing="2" cellpadding="0" align="center">
                <?php
                if($_SESSION['sts-conditional'] == 'Y'){
                ?>
                <tr>
                    <td class="hd">Conditional STS: </td>
                    <td><input type="checkbox" name="txtHold" id="ckHold" onclick="enableDisableHold();"/></td>
                    <td class="hd">On Hold Until: </td>
                    <td><input type="text" name="txtHoldDate" id="txtHoldDate" class="textBox" readonly="readonly"/></td>
                </tr>  
                <?php
                }
                ?>  
