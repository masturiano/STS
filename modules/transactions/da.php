...
switch($_GET['action']){	
	...
	case 'UpdateHdr':
        ...
        $getOnHoldCount = $daObj->getSqlAssoc($daObj->execQry($onHoldCount));
	...
	case 'printSOA':
		...
		if($daObj->createTempTable((int)$_GET['refNo'])){
			if($daObj->checkPayment($_GET['refNo'])=='C'){
				if($daObj->getSequence((int)$_GET['refNo'])){
					$seq = $daObj->getSequence((int)$_GET['refNo']);
					foreach($seq as $val){
						//echo "window.open('report/soa_pdf.php?{$_SERVER['QUERY_STRING']}');";
						echo "window.open('report/soa_da_pdf.php?refNo={$_GET['refNo']}&seq={$val['stsSeq']}');";
					}
				}else{
