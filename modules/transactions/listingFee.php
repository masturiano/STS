...
switch($_GET['action']){	
	...
	case 'printSTS':
		if($listObj->checkPayment($_GET['refNo'])=='C'){
	...
	case 'printSOA':
		...
		if($listObj->createTempTable((int)$_GET['refNo'])){
			if($listObj->checkPayment($_GET['refNo'])=='C'){
				if($listObj->getSequence((int)$_GET['refNo'])){
					$seq = $listObj->getSequence((int)$_GET['refNo']);
					foreach($seq as $val){
						//echo "window.open('report/soa_pdf.php?{$_SERVER['QUERY_STRING']}');";
						echo "window.open('report/soa_listingFee_pdf.php?refNo={$_GET['refNo']}&seq={$val['stsSeq']}');";
					}
				}else{
