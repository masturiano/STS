...
switch($_GET['action']){	
	...
	case 'searchStsRefNo':
		...
		if ($check==0){
			...
			echo "$('#txtComp').html('');\n";
			if($arrSTS['vatTag']=='Y'){
			echo "$('#txtStsAmt').html('". number_format($arrSTS['stsAmt'],2)."<br>With Vat');\n";
			}else{
			...
			echo "$('#txtAutoApproved').html('".$arrSTS['autoTag']."');\n";
	...
	case 'searchSTSSup':
			$arrSTS = $inqObj->getSTSDetSup($_GET['stsNo'],$_GET['suppCode']);
			$upAmt = $inqObj->getAmtSup($_GET['stsNo'],'U','S',$_GET['suppCode']);
			$qAmt = $inqObj->getAmtSup($_GET['stsNo'],'Q','S',$_GET['suppCode']);
			echo "$('#errorMsg').html('');
					$('#errorMsg').removeClass('ui-widget');
					$('#errorMsg').removeClass('ui-state-error');
					$('#errorMsg').removeClass('ui-corner-all');
			...
			if (number_format($arrSTS['stsVatAmt'],2) != "" && number_format($arrSTS['stsVatAmt'],2) > 0){
			echo "$('#txtStsAmt').html('". number_format($arrSTS['stsAmt'],2)."<br>With Vat: ".number_format($arrSTS['stsVatAmt'],2)."');\n";
			}else{
			echo "$('#txtStsAmt').html('". number_format($arrSTS['stsAmt'],2)."');\n";
			}
			echo "$('#txtDept').html('".addslashes($arrSTS['hierarchyDesc'])."');\n";
	...
	case 'PrintContract':
		if($_GET['refNo']==""){
			...
			if($inqObj->checkPayment($_GET['refNo'])=='C'){
				echo "window.open('../transactions/report/stsContent_pdfCollection.php?{$_SERVER['QUERY_STRING']}');";
			}else{
				echo "window.open('../transactions/report/stsContent_pdf.php?{$_SERVER['QUERY_STRING']}');";
			}
		}
	exit();
	break;
	...
	case 'listSupplier':
		$arr = $inqObj->getSuppList($_GET['stsNo']);
		foreach($arr as $val){
			echo "<span onclick=\"searchStsSup('".$_GET['stsNo']."','".$val['suppCode']."');\" class='link'>".$val['suppCode']." - ".$val['ASNAME']."</span> </br>";
		}
	exit();
	break;
}
...
<script type="text/javascript">
	...
	function checkIfDupStsNo(){
		var stsNo = $('#txtSTSNo').val();
		$.ajax({
			url: "sts.php",
			type: "GET",
			traditional: true,
			data: 'action=checkIfDup&stsNo='+stsNo,
			success: function(msg){
	...
	function showSupp(){
		...
		$.ajax({
			...
			type: "GET",
			traditional: true,
			data: 'action=listSupplier&stsNo='+stsNo,
			success: function(msg){
				$("#dialogMsgSupp").html(msg);
			}				
		});	
		$("#dialogSupp").dialog("destroy");
		$("#dialogSupp").dialog({
			modal: true
		});	
	}
	...
	function searchStsSup(stsNo, suppcode){
		clearBox();
		$.ajax({
			url: "sts.php",
			type: "GET",
			traditional: true,
			data: 'action=searchSTSSup&stsNo='+stsNo+'&suppCode='+suppcode,
			beforeSend: function() {
				ProcessData('Searching STS Number...','Open');
			},
			success: function(msg){
     ...
     <table width = "100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <input type="hidden" id="trigger" name="trigger" />
        <tr align="center">
            ...
            <td><input type="text" name="txtSTSNo" id="txtSTSNo" class="textBox  text ui-widget-content ui-corner-all" onkeypress="return valNumInputDec(event)" onkeydown="if(event.keyCode==13) checkIfDupStsNo();"/></td>
        </tr>
       </table>
    </form>
	</fieldset>
...
<div class="headerContent1 ui-corner-all">
        ...
        <table width="100%" border="0"  cellspacing="0" cellpadding="0" class="details">
            ...
            <tr>
   			...
   			<tr>
                ...
                <td colspan="2" id="txtAutoApproved" style="color:red;"></td>
 ...
 <div id='dialogSupp' title='STS'>
        <p id='dialogMsgSupp'></p>
 </div>
</div>
</body>
</html>
