...
FROM         tblStsHdr INNER JOIN
		...
		$sql = "SELECT 
				...
				payMode = CASE tblStsHdr.stsPaymentMode WHEN 'C' THEN 'Collection/Check' ELSE 'Invoice Deduction' END, tblStsHdr.contractNo, tblStsHdr.stsType, 		
				autoTag = CASE tblStsHdr.autoTag WHEN 'Y' THEN 'auto approved' ELSE ' ' END
	...
	function getSTSHdrContract($no){
		$sql = "SELECT     tblStsDtl.stsRefno, tblStsDtl.stsNo, tblStsHdr.dateApproved, tblStsHdr.applyDate, tblStsHdr.nbrApplication, tblStsHdr.suppCode, tblBranches.brnShortDesc, 
...
FROM         tblStsHdr LEFT JOIN
	...
	function getSuppList($no){
		$sql="SELECT     tblStsDtl.stsNo, sql_mmpgtlib.dbo.APSUPP.ASNAME, tblStsHdr.suppCode
FROM         tblStsHdr INNER JOIN
                      tblStsDtl ON tblStsHdr.stsRefno = tblStsDtl.stsRefno INNER JOIN
                      sql_mmpgtlib.dbo.APSUPP ON tblStsHdr.suppCode = sql_mmpgtlib.dbo.APSUPP.ASNUM WHERE tblstsdtl.stsNo = '$no'";
	...
	function getAmtSup($refNo, $stat, $toSearch,$sup){
		if($stat == 'Q'){
			$qry = 'AND stsActualDate is NULL';
		}else{
			$qry = 'AND stsActualDate is not NULL';
		...
		$sql = "SELECT sum(stsApplyAmt) as amt FROM tblstsapply WHERE $ref $qry and suppCode = '$sup'";	
		$amt =  $this->getSqlAssoc($this->execQry($sql));	
		return $amt['amt'];
	}
...
FROM         tblStsApply LEFT OUTER JOIN
	...
	function getSTSDet($no){
		$sql ="SELECT     tblStsDtl.stsRefno, tblStsDtl.stsNo, tblStsHdr.dateApproved, tblStsHdr.applyDate, tblStsHdr.nbrApplication, tblStsHdr.suppCode, tblBranches.brnShortDesc, 
					  ...
					  tblStsHdr.endDate as endDateApp5, tblStsDtl.stsVatAmt,
					  DATEADD(month, tblStsHdr.nbrApplication, tblStsHdr.applyDate) AS endDate, 
					  payMode = CASE tblStsHdr.stsPaymentMode WHEN 'C' THEN 'Collection/Check' ELSE 'Invoice Deduction' END,  			tblStsHdr.stsType,
					  autoTag = CASE tblStsHdr.autoTag WHEN 'Y' THEN 'auto approved' ELSE ' ' END
...
FROM         tblStsDtl INNER JOIN
                      ...
                      tblUsers ON tblStsHdr.enteredBy = tblUsers.userId LEFT JOIN
                      sql_mmpgtlib.dbo.APSUPP ON tblStsHdr.suppCode = sql_mmpgtlib.dbo.APSUPP.ASNUM LEFT JOIN
                      tblUsers tblUsers_1 ON tblStsHdr.approvedBy = tblUsers_1.userId WHERE tblStsDtl.stsNo = '$no' AND tblStsHdr.suppCode = '$sup' and tblStsHierarchy.levelCode = 1";	
		return $this->getSqlAssoc($this->execQry($sql));	
	}
	...
	$sql = "SELECT     tblStsHdr.stsPaymentMode, tblStsDtl.stsNo, tblStsDtl.stsRefno, pg_pf.dbo.tblCompany.compShort, tblStsHdr.contractNo, tblStsHdr.applyDate, DATEADD(month, 
                      tblStsHdr.nbrApplication - 1, tblStsHdr.applyDate) AS endDate, tblStsHdr.dateEntered, tblStsHdr.nbrApplication, tblBranches.brnShortDesc, 
...
FROM         tblStsDtl INNER JOIN
					  ...
					  WHERE tblStsHdr.stsRefno = '$contNo' ";	
		return $this->getArrRes($this->execQry($sql));
	}
...
FROM         tblStsHdr INNER JOIN
	...
	function getUploaded($refNo){
		$sql = "SELECT     tblStsApply.stsNo, tblStsApply.stsSeq, tblBranches.brnShortDesc, tblStsApply.stsApplyAmt
FROM         tblStsApply INNER JOIN
                      tblBranches ON tblStsApply.strCode = tblBranches.strCode WHERE status = 'A' AND stsRefno = '$refNo'";
		return $this->getArrRes($this->execQry($sql));	
	}
	function getOnqueue($refNo){
		$sql = "SELECT     tblStsApply.stsNo, tblStsApply.stsSeq, tblBranches.brnShortDesc, tblStsApply.stsApplyAmt
FROM         tblStsApply INNER JOIN
                      tblBranches ON tblStsApply.strCode = tblBranches.strCode WHERE status IS NULL AND stsRefno = '$refNo'";
		return $this->getArrRes($this->execQry($sql));
	}
