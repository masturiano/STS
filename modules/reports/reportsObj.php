...
FROM         tblCancelledSTS INNER JOIN
					...
					$stsCancelType
                    group by
tblCancelledSTS.stsRefno, tblStsHdr.stsRemarks, tblCancelledSTS.stsNo, tblStsDtl.stsAmt, tblStsDtl.compCode,
                          tblStsDtl.stsVatAmt, tblCancelledSTS.stsSeq, tblCancelledSTS.uploadedAmt, 
                      tblCancelledSTS.queueAmt, tblCancelledSTS.cancelledBy, tblCancelledSTS.cancelDate, tblCancelledSTS.effectivityDate, tblCancelledSTS.stsStrAmt, 
                      sql_mmpgtlib.dbo.APSUPP.ASNAME, tblUsers.fullName, tblCancelType.cancelDesc, tblCancelledSTS.stsPaymentMode,   tblStsHierarchy.hierarchyDesc,
                      tblCancelledSTS.stsType, tblCancelledSTS.replacementSts, tblBranches.brnDesc, tblStsHdr.dateEntered, tblBranches.strCode,
tblStsHdr.stsPaymentMode
				ORDER BY tblCancelledSTS.stsNo";
...
FROM         tblStsHdr INNER JOIN
    ...
    function uploadedArTransmittal(){
        $turnOnAnsiNulls = "SET ANSI_NULLS ON";
        $turnOnAnsiWarn = "SET ANSI_WARNINGS ON";
        $sql="
            select 
            *
            from openquery(pgjda, 'SELECT mmpgtlib.ARZBTC.BTBTCH, mmpgtlib.ARZITN.AROINV, mmpgtlib.ARZITN.OPNREF, mmpgtlib.ARZITN.CUSNUM, mmpgtlib.APSUPP.ASNAME, mmpgtlib.ARZITN.OPNIVD, mmpgtlib.ARZITN.OPNAMT, mmpgtlib.ARZITN.OPNSTR 
            FROM ((mmpgtlib.ARZBTC INNER JOIN mmpgtlib.ARZITN ON mmpgtlib.ARZBTC.BTBTCH = mmpgtlib.ARZITN.ARBTCH) 
            INNER JOIN mmpgtlib.APSREB ON mmpgtlib.ARZITN.CUSNUM = mmpgtlib.APSREB.ASRCUS) 
            INNER JOIN mmpgtlib.APSUPP ON mmpgtlib.APSREB.ASNUM = mmpgtlib.APSUPP.ASNUM
            WHERE 
            mmpgtlib.ARZBTC.BTDESC Like ''%STS WEB%'' 
            And mmpgtlib.ARZITN.OPNSTR Between ''981'' And ''989''
            ') MMS
        "; 
    ...
    function uploadedApTransmittal(){
        $turnOnAnsiNulls = "SET ANSI_NULLS ON";
        $turnOnAnsiWarn = "SET ANSI_WARNINGS ON";
        $sql="
            select MMS.BatchNo, 
            ...
            MMS.originalAmt, 
            MMS.netAmt, 
            MMS.remarks  from openquery(pgjda, 'SELECT MMPGTLIB.APHEAD.APBTCH AS BatchNo, MMPGTLIB.APHEAD.APBDSC AS Description, MMPGTLIB.APIENT.AICMP AS company, MMPGTLIB.APIENT.AISTR AS storeCode, MMPGTLIB.APIENT.AIINV AS invoiceNo, MMPGTLIB.APIENT.AINUM AS vendorNo, MMPGTLIB.APSUPP.ASNAME, MMPGTLIB.APIENT.AIORIG AS originalAmt, MMPGTLIB.APIENT.AITONT AS netAmt, MMPGTLIB.APIENT.AINOTE AS remarks
            FROM (MMPGTLIB.APHEAD INNER JOIN MMPGTLIB.APIENT ON MMPGTLIB.APHEAD.APBTCH = MMPGTLIB.APIENT.AIEBCH) INNER JOIN MMPGTLIB.APSUPP ON MMPGTLIB.APIENT.AINUM = MMPGTLIB.APSUPP.ASNUM
            WHERE (((MMPGTLIB.APHEAD.APBDSC) Like ''%STS WEB%'')) 
            And MMPGTLIB.APIENT.AICMP > 700
            ORDER BY MMPGTLIB.APIENT.AISTR, MMPGTLIB.APIENT.AIINV') MMS
        "; 
        $this->execQry($turnOnAnsiNulls);
        $this->execQry($turnOnAnsiWarn);
        return $this->getArrRes($this->execQry($sql));  
    ...
    function uploadedAp2Transmittal(){
        $turnOnAnsiNulls = "SET ANSI_NULLS ON";
        $turnOnAnsiWarn = "SET ANSI_WARNINGS ON";
        $sql="
        Select MMS.BatchNo, 
        ...
        MMS.remarks, 
        MMS.invoiceDate
        from openquery(pgjda, 'SELECT MMPGTLIB.APHEAD.APBTCH AS BatchNo, MMPGTLIB.APHEAD.APBDSC AS Description, MMPGTLIB.APIENT.AICMP AS company, MMPGTLIB.APIENT.AISTR AS storeCode, MMPGTLIB.APIENT.AIINV AS invoiceNo, MMPGTLIB.APIENT.AINUM AS vendorNo, MMPGTLIB.APSUPP.ASNAME, MMPGTLIB.APIENT.AIORIG AS originalAmt, MMPGTLIB.APIENT.AITONT AS netAmt, MMPGTLIB.APIENT.AINOTE AS remarks, MMPGTLIB.APIENT.AIDTIV AS invoiceDate
        FROM (MMPGTLIB.APHEAD INNER JOIN MMPGTLIB.APIENT ON MMPGTLIB.APHEAD.APBTCH = MMPGTLIB.APIENT.AIEBCH) 
        INNER JOIN MMPGTLIB.APSUPP ON MMPGTLIB.APIENT.AINUM = MMPGTLIB.APSUPP.ASNUM
        WHERE (((MMPGTLIB.APHEAD.APBDSC) Like ''STS/DAC%'')) 
        And MMPGTLIB.APIENT.AICMP <> 700
        ORDER BY MMPGTLIB.APIENT.AISTR, MMPGTLIB.APIENT.AIINV
        ') MMS   
        "; 
        $this->execQry($turnOnAnsiNulls);
        $this->execQry($turnOnAnsiWarn);
    ...
    function uploadedApNeTransmittal(){
        $turnOnAnsiNulls = "SET ANSI_NULLS ON";
        $turnOnAnsiWarn = "SET ANSI_WARNINGS ON";
        $sql="
        select MMS.BatchNo, 
        ...
        MMS.netAmt, 
        MMS.remarks,
        MMS.invoiceDate  from openquery(pgjda, 'SELECT MMPGTLIB.APHEAD.APBTCH AS BatchNo, MMPGTLIB.APHEAD.APBDSC AS Description, MMPGTLIB.APIENT.AICMP AS company, MMPGTLIB.APIENT.AISTR AS storeCode, MMPGTLIB.APIENT.AIINV AS invoiceNo, MMPGTLIB.APIENT.AINUM AS vendorNo, MMPGTLIB.APSUPP.ASNAME, MMPGTLIB.APIENT.AIORIG AS originalAmt, MMPGTLIB.APIENT.AITONT AS netAmt, MMPGTLIB.APIENT.AINOTE AS remarks, MMPGTLIB.APIENT.AIDTIV AS invoiceDate
        FROM (MMPGTLIB.APHEAD INNER JOIN MMPGTLIB.APIENT ON MMPGTLIB.APHEAD.APBTCH = MMPGTLIB.APIENT.AIEBCH) INNER JOIN MMPGTLIB.APSUPP ON MMPGTLIB.APIENT.AINUM = MMPGTLIB.APSUPP.ASNUM
        WHERE (((MMPGTLIB.APHEAD.APBDSC) Like ''%STS/DAC%'')) 
        And MMPGTLIB.APIENT.AICMP in (''810'',''811'')
        ORDER BY MMPGTLIB.APIENT.AISTR, MMPGTLIB.APIENT.AIINV') MMS
        "; 
        $this->execQry($turnOnAnsiNulls);
        $this->execQry($turnOnAnsiWarn);
    ...
    function uploadedArNeTransmittal(){
        $turnOnAnsiNulls = "SET ANSI_NULLS ON";
        $turnOnAnsiWarn = "SET ANSI_WARNINGS ON";
        $sql="
            select 
            *
            ...
            INNER JOIN mmpgtlib.APSUPP ON mmpgtlib.APSREB.ASNUM = mmpgtlib.APSUPP.ASNUM
            WHERE 
            mmpgtlib.ARZBTC.BTDESC Like ''%STS WEB%'' and
            mmpgtlib.ARZITN.OPNSTR in (''990'',''990'')
            ') MMS
        "; 
        $this->execQry($turnOnAnsiNulls);
        $this->execQry($turnOnAnsiWarn);
    ...
    //Date: March 27, 2015
    //Programmer: Mydel-Ar Asturiano
    //SSRF No.: 14-000499
    //Requestor: Bernadeth Olfato
    //Departmet: FCD
    
    function getApprovedSTSAP($dtFrom,$dtTo,$status,$str,$trans){
        if($status =='O'){
            $stat = "AND (tblStsApply.status  IS NULL)";
        }elseif($status == 'A'){
            $stat = "AND tblStsApply.status  = 'A'";
        ...
        tblStsApply.stsType, 
        tblStsApply.stsNo, 
        tblStsApply.stsApplyAmt, 
        tblBranches.brnDesc, 
        sql_mmpgtlib.dbo.APSUPP.ASNAME AS suppName, 
        tblStsApply.stsSeq, 
        tblStsApply.stsApplyDate, 
        tblStsApply.stsRefno, 
        tblStsHdr.nbrApplication, 
        tblStsHdr.enteredBy, 
        tblStsHdr.stsRemarks, 
        tblStsDlyApHist.apBatch, 
        tblStsApply.stsVatAmt,
        tblStsDlyArHist.arBatch, 
        payMode = CASE tblStsHdr.stsPaymentMode WHEN 'C' THEN 'Collection/Check' ELSE 'Invoice Deduction' END, 
        applyStatus = CASE tblStsApply.status WHEN NULL THEN 'ONQUEUE' ELSE 'APPLIED' END,
        tblStsHdr.contractNo,
        tblStsApply.compCode,
        tblStsHdr.dateEntered,
        tblStsHdr.applyDate,
        endDAte = CASE WHEN tblStsHdr.ststype = 5 THEN endDate ELSE DATEADD(month, tblStsHdr.nbrApplication, tblStsHdr.applyDate) END,
        tblStsHdr.dateApproved,
        tblStsHdr.approvedBy,
        tblUsers.fullName as approveByName,
        tblGroup.grpDesc,
        tblStsHierarchy.hierarchyDesc
        FROM         tblStsApply 
        INNER JOIN tblBranches ON tblStsApply.compCode = tblBranches.compCode AND tblStsApply.strCode = tblBranches.strCode 
        LEFT OUTER JOIN sql_mmpgtlib.dbo.APSUPP ON tblStsApply.suppCode = sql_mmpgtlib.dbo.APSUPP.ASNUM 
        left outer JOIN tblStsDlyArHist ON tblStsApply.stsNo = tblStsDlyArHist.stsNo 
            AND tblStsApply.stsSeq = tblStsDlyArHist.stsSeq 
            AND tblStsApply.stsRefno = tblStsDlyArHist.stsRefno 
        LEFT OUTER JOIN tblStsDlyApHist ON tblStsApply.stsNo = tblStsDlyApHist.stsNo AND tblStsApply.stsSeq = tblStsDlyApHist.stsSeq 
            AND tblStsApply.stsRefno = tblStsDlyApHist.stsRefno 
        LEFT OUTER JOIN tblStsHdr ON tblStsApply.stsRefno = tblStsHdr.stsRefno
        left JOIN tblStsHierarchy ON tblStsHdr.stsDept = tblStsHierarchy.stsDept AND tblStsHdr.stsCls = tblStsHierarchy.stsCls
        left  JOIN tblGroup ON tblStsHdr.grpCode = tblGroup.grpCode
        left  JOIN tblUsers ON userId = tblStsHdr.approvedBy
        WHERE tblStsHdr.dateApproved BETWEEN '$dtFrom' AND '$dtTo'
        AND tblBranches.strCode = '$str' 
        $stat  
        $stsType 
        ORDER BY tblStsApply.status
        ";          
        // AND sql_mmpgtlib.dbo.APSUPP.ASNUM = '$suppCode' 
        return $this->getArrRes($this->execQry($sql));
    }
    
    function getApprovedSTSAPSup($dtFrom,$dtTo,$status,$suppCode,$trans){
        if($status =='O'){
            $stat = "AND (tblStsApply.status IS NULL)";
        }elseif($status == 'A'){
            $stat = "AND tblStsApply.status = 'A'";
