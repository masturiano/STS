...
################### INCLUDE FILE #################
	...
	include("monthlyStsSummaryPerStoreObj.php");
	require_once 'Spreadsheet/Excel/Writer.php';
	
	$monthlyStsSummaryPerStoreObj = new monthlyStsSummaryPerStoreObj();
	...
	$filename = "Monthly_STS_summary_per_store.xls";
	$workbook->send($filename);
	$worksheet = &$workbook->addWorksheet("transaction_details");
	$worksheet->setLandscape();
	$worksheet->freezePanes(array(4,0));

	$worksheet->write(0,0,"Monthly STS Summary per Store From ".date('M-Y',strtotime($_GET['txtDateFrom']))." to ".date('M-Y',strtotime($_GET['txtDateTo'])),$headerFormat);
	for($i=1;$i<=1;$i++) 
	{
	...
	for($iCol=2;$iCol<=100;$iCol++) {
		...
		$arrDet = $monthlyStsSummaryPerStoreObj->stsSummPerStoreDtlLasYear($_GET['txtDateFrom'],$_GET['txtDateTo'],$arrMonth);
		$arrDet2 = $monthlyStsSummaryPerStoreObj->stsSummPerStoreDtlCurYear($_GET['txtDateFrom'],$_GET['txtDateTo'],$arrMonth);
		...
		foreach ($arrDet as $valDet) {
			...
			$worksheet->write($ctr,0,$valDet['strCode'],$number);
			$worksheet->write($ctr,1,$valDet['brnDesc'],$detail);
