...
################### INCLUDE FILE #################
	...
	$worksheet->write(0,0,"$status $type Approved Date From ".date('m/d/Y',strtotime($_GET['txtDateFrom']))." to ".date('m/d/Y',strtotime($_GET['txtDateTo']))." By Branch",$headerFormat);
	for($i=1;$i<=10;$i++) {
	...
	$worksheet->write(2,6,"VAT AMOUNT",$headerFormat);
	$worksheet->write(2,7,"PAYMENT MODE",$headerFormat);
	$worksheet->write(2,8,"APPLY DATE",$headerFormat);
	$worksheet->write(2,9,"NO. OF APPLICATION",$headerFormat);
    $worksheet->write(2,10,"STATUS",$headerFormat);
	$worksheet->write(2,11,"MMS REF",$headerFormat);
    $worksheet->write(2,12,"CONTRACT NO",$headerFormat);
    $worksheet->write(2,13,"COMPANY",$headerFormat);
    $worksheet->write(2,14,"DATE ENTERED",$headerFormat);
    $worksheet->write(2,15,"APP START DATE",$headerFormat);
    $worksheet->write(2,16,"END DATE",$headerFormat);
    $worksheet->write(2,17,"DATE APPROVED",$headerFormat);
    $worksheet->write(2,18,"APPROVED BY",$headerFormat);
    $worksheet->write(2,19,"HEIRARCHY",$headerFormat);
    $worksheet->write(2,20,"GROUP",$headerFormat);
		...
		$arrTran = $reportsObj->getStsBranch($_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbStatus'],$_GET['cmbTran']);
			...
			foreach ($arrTran as $val) {
				...
				$col = ($col==0) ? 1:0;
				$arrH = $reportsObj->getApprovedSTSAP($_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbStatus'],$val['strCode'],$_GET['cmbTran']);
				$ctr++;
				...
				foreach($arrH as $valD){
					...
					$worksheet->write($ctr,6,number_format($valD['stsVatAmt'],2),$row2);
					$worksheet->write($ctr,7,$valD['payMode'],$row2);
					$worksheet->write($ctr,8,date('m/d/Y',strtotime($valD['stsApplyDate'])),$row2);
					$worksheet->write($ctr,9,$valD['nbrApplication'],$row2);
					$worksheet->write($ctr,10,$valD['applyStatus'],$row);
					
					if($valD['stsPaymentMode'] =='D'){
						$worksheet->write($ctr,11,$valD['apBatch'],$row);	
					}else{
						$worksheet->write($ctr,11,$valD['arBatch'],$row);	
					}
                    $worksheet->write($ctr,12,$valD['contractNo'],$row);
                    $worksheet->write($ctr,13,$valD['compCode'],$row);
                    $worksheet->write($ctr,14,date('m/d/Y',strtotime($valD['dateEntered'])),$row2);
                    $worksheet->write($ctr,15,date('m/d/Y',strtotime($valD['applyDate'])),$row2);
                    $worksheet->write($ctr,16,date('m/d/Y',strtotime($valD['endDAte'])),$row2);
                    $worksheet->write($ctr,17,date('m/d/Y',strtotime($valD['dateApproved'])),$row2);
                    $worksheet->write($ctr,18,$valD['approveByName'],$row);
                    $worksheet->write($ctr,19,$valD['hierarchyDesc'],$row);
                    $worksheet->write($ctr,20,$valD['grpDesc'],$row);
