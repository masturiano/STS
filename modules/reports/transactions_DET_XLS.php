...
################### INCLUDE FILE #################
	...
	*/
	...
	$worksheet->write(2,19,"Hierarchy Desc",$headerFormat);
	//if($_GET['cmbTran'] == '5'){
				...
				/*foreach($arrH as $valD){
					...
					*/
					...
					foreach ($arrDet as $valDet) {
						...
						$worksheet->write($ctr,13,'',$row3);
						}
						$worksheet->write($ctr,14,$valDet['fullName'],$row2);
						$worksheet->write($ctr,15,$valDet['hierarchyDesc'],$row);
						$worksheet->write($ctr,16,$valDet['grpDesc'],$row);
						$worksheet->write($ctr,17,substr($valDet['stsRemarks'],0,45),$row);
						$worksheet->write($ctr,18,substr($valDet['glCode'],0,45),$row);
						$worksheet->write($ctr,19,substr($valDet['hierarchyDesc'],0,45),$row);
