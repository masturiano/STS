...
################### INCLUDE FILE #################
	...
	$worksheet->setColumn(0,2,9);
	$worksheet->setColumn(0,3,8);
	$worksheet->setColumn(0,4,29);
	$worksheet->setColumn(0,5,12);
	$worksheet->setColumn(0,6,9);
	$worksheet->setColumn(0,7,12);
	$worksheet->setColumn(0,8,18);
	$worksheet->setColumn(0,9,10);
	$worksheet->setColumn(0,10,17);
	$worksheet->setColumn(0,11,17);
	$worksheet->setColumn(0,12,14);
	$worksheet->setColumn(0,13,20);
	$worksheet->setColumn(0,14,20);
	$worksheet->setColumn(0,15,25);
	$worksheet->setColumn(0,16,23);
	$worksheet->setColumn(0,17,45);
	$worksheet->setColumn(0,18,17);
	...
	*/
	...
	$worksheet->write(2,6,"Vat Amt.",$headerFormat);
	...
	$worksheet->write(2,10,"Mode of Payment",$headerFormat);
	$worksheet->write(2,11,"Contract No",$headerFormat);
	$worksheet->write(2,12,"Date Approved",$headerFormat);
	$worksheet->write(2,13,"Approved By",$headerFormat);
	$worksheet->write(2,14,"Hierarchy",$headerFormat);
	$worksheet->write(2,15,"Group",$headerFormat);
	$worksheet->write(2,16,"Display Specs",$headerFormat);
	$worksheet->write(2,17,"Remarks",$headerFormat);
	$worksheet->write(2,18,"Hierarchy Desc",$headerFormat);
				...
				/*foreach($arrH as $valD){
					...
					*/
					...
					foreach ($arrDet as $valDet) {
						...
						if($valDet['stsPaymentMode']=="C"){
						$worksheet->write($ctr,6,number_format($valDet['stsVatAmt'],2),$row2);
						}else{
						$worksheet->write($ctr,6,number_format('0',2),$row2);	
						}
						...
						$worksheet->write($ctr,10,$valDet['payMode'],$row);
						$worksheet->write($ctr,11,$valDet['contractNo'],$row);
						if($valDet['stsNo'] != ""){
						$worksheet->write($ctr,12,date('m/d/Y',strtotime($valDet['dateApproved'])),$row3);
						}else{
						$worksheet->write($ctr,12,'',$row3);
						}
						$worksheet->write($ctr,13,$valDet['fullName'],$row2);
						$worksheet->write($ctr,14,$valDet['hierarchyDesc'],$row);
						$worksheet->write($ctr,15,$valDet['grpDesc'],$row);
						$worksheet->write($ctr,16,$valDet['displaySpecsDesc'],$row);
						$worksheet->write($ctr,17,substr($valDet['stsRemarks'],0,45),$row);
						$worksheet->write($ctr,18,substr($valDet['hierarchyDesc'],0,45),$row);
