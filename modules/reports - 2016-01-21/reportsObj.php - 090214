...
WHERE $dt  BETWEEN '$dtStart' AND '$dtEnd' $stsType $filter1 $filter2 $grpVar AND tblStsHdr.stsRefno in (SELECT distinct stsRefno FROM tblStsDtl) ORDER BY pg_pf.dbo.tblStsHdr.suppCode ";	
		return $this->getArrRes($this->execQry($sql));
	}
	...
	function getParDetail3($trans,$dtStart,$dtEnd,$stat,$suppCode,$grp){
		...
		$stsType";	*/	
		 $sql = "SELECT     tblStsDtl.stsRefno, tblStsHdr.suppCode + ' - ' + sql_mmpgtlib.dbo.APSUPP.ASNAME AS supplier, tblStsDtl.stsNo, 
                      tblBranches.strCode + ' - ' + tblBranches.brnShortDesc AS branch, tblStsDtl.stsAmt, tblStsHdr.dateEntered, tblStsHdr.applyDate, 
