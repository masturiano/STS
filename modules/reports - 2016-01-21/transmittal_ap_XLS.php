...
################### INCLUDE FILE #################
    ...
    $filename = "Parco AP For posting.xls";
    $workbook->send($filename);
    $worksheet = &$workbook->addWorksheet("Ap");
    $worksheet->setLandscape();
    $worksheet->freezePanes(array(3,0));

    $worksheet->write(0,0,"Parco AP For posting",$headerFormat);
    for($i=1;$i<=9;$i++) {
        if($i!=2 || $i!=3){
    ...
    $worksheet->write(2,9,"remarks",$headerFormat);
        ...
        $ctr = 3 ;
        $arrTran = $reportsObj->uploadedApTransmittal();
            ...
            //$worksheet->write($ctr,7,number_format($totNetAmt,2),$headerFormat);
