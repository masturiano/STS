...
################### INCLUDE FILE #################
	...
	$filename = "302 with parco or company AR for posting.xls";
	$workbook->send($filename);
	$worksheet = &$workbook->addWorksheet("Ar");
	$worksheet->setLandscape();
	$worksheet->freezePanes(array(3,0));

	$worksheet->write(0,0,"302 with parco or company AR for posting",$headerFormat);
	for($i=1;$i<=7;$i++) {
		if($i!=2 || $i!=3){
	...
	$worksheet->write(2,7,"OPNSTR",$headerFormat);
		...
		$ctr = 3 ;
		$arrTran = $reportsObj->uploadedArTransmittal();
