...
################### INCLUDE FILE #################
	...
	$worksheet->write(2,0,"Invoice No.",$headerFormat);
	$worksheet->write(2,1,"Amount",$headerFormat);
	$worksheet->write(2,2,"Vat Amount",$headerFormat);
	$worksheet->write(2,3,"Supplier",$headerFormat);
	$worksheet->write(2,4,"Store",$headerFormat);
	$worksheet->write(2,5,"Hierarchy",$headerFormat);
	$worksheet->write(2,6,"Apply Date",$headerFormat);
	$worksheet->write(2,7,"Upload Date",$headerFormat);
	}
		...
		$arrTran = $reportsObj->uploadedTransmittal($_GET['cmbType'], $_GET['cmbTran'],$_GET['cmbComp'],$_GET['txtDateFrom'],$_GET['txtDateTo']);
				if($_GET['cmbType']=='AP'){
					...
					$col = ($col==0) ? 1:0;
						...
						$worksheet->write($ctr,2,number_format($valD['stsVatAmt'],2),$row2);
						}else{
						$worksheet->write($ctr,2,'0',$row2);
						}
						$worksheet->write($ctr,3,$valD['Supplier'],$row);
						$worksheet->write($ctr,4,$valD['Store'],$row);
						$worksheet->write($ctr,5,$valD['hierarchyDesc'],$row);
						$worksheet->write($ctr,6,date('m/d/Y',strtotime($valD['stsApplyDate'])),$row3);
						$worksheet->write($ctr,7,date('m/d/Y',strtotime($valD['uploadDate'])),$row3);
