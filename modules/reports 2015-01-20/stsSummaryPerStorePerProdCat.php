...
switch($_GET['action']){	
	...
	case 'PrintXls':
			if($_GET['cmbType'] != 3){
				echo "window.open('stsSummaryPerStorePerProdCat_XLS.php?{$_SERVER['QUERY_STRING']}');";
			}else{
				echo "window.open('stsSummaryPerStorePerProdCatPf_XLS.php?{$_SERVER['QUERY_STRING']}');";
			}
...
<script type="text/javascript">
	$(function(){		
		...
		$("#txtSup").autocomplete({
			source: "stsSummaryPerStorePerProdCat.php?action=searchSupplier",
			minLength: 1,
		...
		$("#printInqXls").click( function (){
			...
			if (valDateStartEnd(dateStart,dateEnd,'txtDateFrom','txtDateTo')){
				if (cmbType == 0){
					...
					$.ajax({
						url: 'stsSummaryPerStorePerProdCat.php',
						type: "GET",
...
<h2 class="ui-widget-header ui-corner-all" style="padding:5px;">STS SUMMARY PER STORE PER PRODUCT CATEGORY</h2>
