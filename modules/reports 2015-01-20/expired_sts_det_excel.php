...
################### INCLUDE FILE #################
	...
	$worksheet->write(2,4,"STS VAT AMT.",$headerFormat);
	...
	$worksheet->write(2,8,"ENTERED BY",$headerFormat);
		...
		$arrExp = $reportsObj->expiredContractsSumm(date('m/d/Y',strtotime($_GET['dtMonthYr'])),$_GET['cmbTran']);
			...
			foreach ($arrExp as $val) {
				...
				if($val['stsPaymentMode']=='C'){
					if (number_format($val['uploadedVatAmt'],2) != "" && number_format($val['uploadedVatAmt'],2) > 0){
					$worksheet->write($ctr,4,number_format($val['uploadedVatAmt'],2),$row2);
					}else{
					$worksheet->write($ctr,4,'0',$row2);
					}
				}else{
				$worksheet->write($ctr,4,'0',$row2);
				}
				$worksheet->write($ctr,5,number_format($val['uploadedAmt'],2),$row2);
				$worksheet->write($ctr,6,number_format($val['queueAmt'],2),$row2);
				$worksheet->write($ctr,7,$val['stsStartNo']." - ".$val['stsEndNo'],$row2);
				...
				$worksheet->write($ctr,4,"Store Vat Amount",$row2);
				$worksheet->write($ctr,5,"Uploaded Amount",$row2);
				$worksheet->write($ctr,6,"Onqueue Amount",$row2);
				$worksheet->write($ctr,7,"Group",$row);
				$worksheet->write($ctr,8,"Hierarchy",$row);
				...
				foreach ($arrDtl as $valD) {
					...
					if($valD['stsPaymentMode']=='C'){
						if (number_format($valD['stsVatAmt'],2) != "" && number_format($valD['stsVatAmt'],2) > 0){
						$worksheet->write($ctr,4,number_format($valD['stsVatAmt'],2),$row2);
						}else{
						$worksheet->write($ctr,4,'0',$row2);
						}
					}else{
					...
					$worksheet->write($ctr,5,number_format($valD['uploadedAmt'],2),$row2);
					$worksheet->write($ctr,6,number_format($valD['queueAmt'],2),$row2);
					$worksheet->write($ctr,7,$valD['grpDesc'],$row);
					$worksheet->write($ctr,8,$valD['hierarchyDesc'],$row);
					$totStsD += $valD['stsAmt'];
					$totVatStsD += $valD['stsVatAmt'];
					$totUploadD += $valD['uploadedAmt'];
					$totQueueD += $valD['queueAmt'];
					
					$totSts += $valD['stsAmt'];
					$totVatSts += $valD['stsVatAmt'];
				...
				$worksheet->write($ctr,4,number_format($totVatStsD,2),$headerFormat);
				$worksheet->write($ctr,5,number_format($totUploadD,2),$headerFormat);
				$worksheet->write($ctr,6,number_format($totQueueD,2),$headerFormat);
				unset($totStsD,$totVatStsD,$totUploadD,$totQueueD);
			}
			$ctr++;	
			$worksheet->setRow($ctr,16);
			$worksheet->write($ctr,2,"Grand Total",$headerFormat);
			$worksheet->write($ctr,3,number_format($totSts,2),$headerFormat);
			$worksheet->write($ctr,4,number_format($totVatSts,2),$headerFormat);
