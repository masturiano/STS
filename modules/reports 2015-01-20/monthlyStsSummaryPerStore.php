...
switch($_GET['action']){	
	...
	case 'PrintXls':
			echo "window.open('monthlyStsSummaryPerStore_XLS.php?{$_SERVER['QUERY_STRING']}');";
	exit();
	break;
...
<script type="text/javascript">
	$(function(){		
		...
		$("#txtSup").autocomplete({
			source: "monthlyStsSummaryPerStore.php?action=searchSupplier",
			minLength: 1,
		...
		$("#printInq").click( function (){
			...
			if (valDateStartEnd(dateStart,dateEnd,'txtDateFrom','txtDateTo')){
				$.ajax({
					url: 'monthlyStsSummaryPerStore.php',
					type: "GET",
...
<h2 class="ui-widget-header ui-corner-all" style="padding:5px;">MONTHLY STS SUMMARY PER STORE</h2>
